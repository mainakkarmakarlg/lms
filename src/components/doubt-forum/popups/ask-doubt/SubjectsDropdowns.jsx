...
const SubjectsDropdowns = ({ setError }) => {
  ...
  const handleSubjectChange = (data) => {
    ...
    const fallId = findFallNumId(data, allSubjects);
    if (fallId) {
      dispatch(setFallId(fallId));
    }
  ...
  const handleTopicChange = (data) => {
    ...
    const fallId = findFallNumId(data, allTopics);
    if (fallId) {
      dispatch(setFallId(fallId));
    }
  ...
  const handlePointChange = (data) => {
    const fallId = findFallNumId(data, allPoints);
